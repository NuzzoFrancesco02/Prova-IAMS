...
D_t = timeOfFlight(ai,ei,th_i,th_peri_1,mu) + timeOfFlight(ai,ei,th_peri_2,0,mu);
th_man_1 = u_i-om_cp;
th_man_2 = th_man_1;
om2 = u_f - th_man_2 + 2*pi;
% bitangente
...
D_t = D_t + timeOfFlight(abit1,ebit1,0,pi,mu) + timeOfFlight(abit2,ebit2,pi,2*pi,mu);
% cambio periasse finale (prima del cambio forma poiche e è più bassa)
d_om = om_f-om2+2*pi;
th_peri_1 = d_om/2; th_peri_2 = 2*pi-d_om/2;
D_v = D_v + abs(2*sqrt(mu./pf).*ef*sin(d_om/2)); %D_v cambio periasse
D_t = D_t+timeOfFlight(af,ef,0,th_peri_1,mu) + timeOfFlight(af,ef,th_peri_2,th_f,mu);
figure()
...
hold on;
yyaxis right
plot(r_bit,D_t./3600,'LineWidth',2)
ylabel('t [h]');
xlabel('r_{man} [Km]');  grid on;